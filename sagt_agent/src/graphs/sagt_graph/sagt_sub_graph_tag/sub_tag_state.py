...
本文件定义了客户标签子图的状态结构，包括输入状态、中间状态和输出状态。
...
class SubTagStateField(str, Enum):
    ...
    """
    # 输入字段 - 从主图状态继承
    TAG_SETTING = SagtStateField.TAG_SETTING.value         # 系统标签设置配置
    ...
    SUGGESTION_TAG = SagtStateField.SUGGESTION_TAG.value   # AI生成的标签建议
...
class SubTagInputState(TypedDict):
    ...
    """
    tag_setting:        TagSetting        # 系统可用标签配置
...
class SubTagIntermediateState(TypedDict):
    ...
    suggestion_tag:     TagSuggestion   # AI生成的标签变更建议
