...
本文件定义了客户聊天建议子图的状态结构，包括输入状态、中间输出状态和输出状态。
子图状态与主图状态通过字段名称映射实现数据共享。

业务背景：该子图用于为销售人员生成与客户聊天的回复建议，
基于聊天历史、客户信息等数据，调用LLM生成合适的回复内容。
"""
...
class SubChatSuggestionStateField(str, Enum):
    ...
    # 输入字段 - 从主图状态继承
    EMPLOYEE_INFO    = SagtStateField.EMPLOYEE_INFO.value     # 当前员工信息
    ...
    CUSTOMER_PROFILE = SagtStateField.CUSTOMER_PROFILE.value  # 客户画像

    # 中间输出字段 - 生成的建议结果
    SUGGESTION_CHAT     = SagtStateField.SUGGESTION_CHAT.value  # AI生成的聊天回复建议
...
class SubChatSuggestionInputState(TypedDict):
    ...
    """
    employee_info:      EmployeeInfo    # 当前操作员工的信息
...
class SubChatSuggestionIntermediateOutputState(TypedDict):
    ...
    定义了子图执行过程中生成的中间结果。
    """
    suggestion_chat:    ReplySuggestion # AI生成的聊天回复建议
